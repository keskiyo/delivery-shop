...
| **API-доки**         | Swagger UI (OpenAPI 3.0.3) на `/api-docs`, спека — `/api/docs`, аннотации во всех роутах  |
...
- **API-документация:** Swagger / OpenAPI через `next-swagger-doc` + `swagger-jsdoc`, UI из `swagger-ui-dist` (CDN)
...
Интерактивная документация API — **Swagger UI** на [http://localhost:3000/api-docs](http://localhost:3000/api-docs).

- Спецификация OpenAPI 3.0.3 отдаётся эндпоинтом `GET /api/docs` (JSON).
- Спека собирается из JSDoc-аннотаций `@swagger` над хендлерами в `src/app/api/**/route.ts`.
- Генератор — [`src/lib/swagger.ts`](src/lib/swagger.ts) (`next-swagger-doc`); UI грузится из `swagger-ui-dist` по CDN, поэтому нет конфликтов peer-зависимостей с React 19.
- «Try it out» отправляет cookie-сессию (`withCredentials`) — защищённые роуты можно вызывать из-под залогиненного браузера.

При добавлении нового API-роута добавляй блок `@swagger` над хендлером — он автоматически попадёт в документацию.
