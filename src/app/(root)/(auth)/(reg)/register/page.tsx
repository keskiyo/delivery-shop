 ...
 * 
 * Функционал:
 * - Форма регистрации с обязательными и необязательными полями
 * - Валидация всех полей формы
 * - Выбор метода верификации (email или телефон)
 ...
 * 
 * Необязательные поля:
 * - Email (если указан, предлагается выбор метода верификации)
 * - Номер карты лояльности или чекбокс "Получить карту"
 * 
 * Логика работы:
 * 1. Пользователь заполняет форму
 * 2. При отправке валидируются все поля
 * 3. Данные сохраняются в контекст RegFormContext
 * 4. Если указан email - показывается модальное окно выбора метода верификации
 * 5. Если email не указан - редирект на верификацию по телефону
 * 
 * @route /register
 */
const RegisterPage = () => {
	...
	return (
		<AuthFormLayout variant='register'>
			...
			>
				...
				{invalidFormMessage && (
					<div className='text-danger text-center my-4 p-4 bg-danger-soft rounded'>
						{invalidFormMessage}
					</div>
				)}
				<RegFormFooter
