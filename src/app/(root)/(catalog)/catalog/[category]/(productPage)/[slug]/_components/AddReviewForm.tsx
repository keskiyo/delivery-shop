...
/**
 * Компонент формы добавления отзыва о товаре
 * 
 * Функционал:
 * - Выбор рейтинга (1-5 звезд) с hover эффектом
 * - Ввод текста отзыва
 * - Валидация формы (рейтинг и текст обязательны)
 * - Отправка отзыва на сервер
 * - Обработка ошибок и состояний загрузки
 * 
 * Логика работы:
 * 1. Пользователь выбирает рейтинг (клик по звездам)
 * 2. Вводит текст отзыва (минимум 1 символ)
 * 3. При отправке проверяется авторизация
 * 4. Валидируется форма (рейтинг > 0 и текст не пустой)
 * 5. Отправляется POST запрос на /api/products/{productId}/reviews
 * 6. После успеха форма очищается и вызывается callback onReviewAdded
 * 
 * Валидация:
 * - Рейтинг: обязателен, от 1 до 5
 * - Комментарий: обязателен, минимум 1 символ (без пробелов)
 * - Авторизация: обязательна
 * 
 * Особенности:
 * - Hover эффект на звездах (подсветка при наведении)
 ...
 * 
 * @param productId - ID товара для которого добавляется отзыв
 * @param onReviewAdded - Callback вызываемый после успешного добавления отзыва
 */
