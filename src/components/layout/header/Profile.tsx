...
/**
 * Компонент профиля пользователя в header
 *
 * Функционал:
 * - Неавторизованный пользователь: кнопка "Войти" + переключатель темы
 * - Авторизованный пользователь: аватар + имя + выпадающее меню
 * - Выпадающее меню: Профиль, Главная, Панель управления (для admin/manager), Выйти
 *
 * Логика:
 * - При клике на аватар открывается/закрывается меню
 * - При клике вне меню оно закрывается
 * - Аватар загружается с сервера или используется дефолтный по полу
 * - Текст отображает имя или роль (Администратор/Менеджер)
 *
 * Используется в:
